...
export default function MyPortfolio() {
  ...
  const [isHoveringImg, setIsHoveringImg] = useState(-1)
  ...
  return (
    <section id="myPortfolio" className="portfolioSection">
      <div className="portfolioContainerBox">
        ...
        <div className="portfolioBtnHoverWrapper">
          <button type="button" className="btn btnGithub" onMouseOver={handleMouseOver}
          onMouseOut={handleMouseOut} onClick={()=>openInNewTab("https://github.com/Yivgha")}>
            <svg
          ...
          {isHovering && (
          <p className="portfolioBtnHoverText">Click to open in new window</p>
        )}
        </div>
      </div>
      <div className="portfolioSectionContainer">
        {data?.portfolio?.map((item, index) => (
          <div key={index} className="portfolioSectionCard">
            <div className="portfolioSectionImg" onClick={() => openInNewTab(item.liveURL)} 
              onMouseEnter={() => handleMouseIn(index)}
              onMouseLeave={handleMouseAway}
         >
              <img src={item.src} alt={item.title} className="portfolioIMG"/>
              { isHoveringImg === index && (
                <p className="portfolioImgHoverText">Open url: {item.liveURL}</p>
        )}
            </div>
            <div className="portfoiloSectionCardContent" >
              <div>
                <h3 className="portfolioSectionTitle">{item.title}</h3>
                <p className="textMD">{item.description}</p>
              </div>
              <p className="textSM portfolioLink">
